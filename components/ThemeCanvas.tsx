...
type ThemeType = 'none' | 'astro' | 'game' | 'cyberpunk'
    ...
    const loop = () => {
      ...
      if (theme === 'astro') {
        ...
        if (shootingStar.active) {
          ...
          const grad = ctx.createLinearGradient(shootingStar.x, shootingStar.y, shootingStar.x + shootingStar.length, shootingStar.y - shootingStar.length)
        ...
        if (gameGlitch.active) {
          ...
          if (gameGlitch.timer % 3 === 0) {
            ...
            ctx.fillRect(Math.random() * width, Math.random() * height, Math.random() * 200 + 50, Math.random() * 10 + 2)
          }
